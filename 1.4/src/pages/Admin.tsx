            ...
            {posts.map(p => (
              <tr key={p.id} className="border border-[#7f9db9] bg-white">
                ...
                <td className="p-1 border border-[#7f9db9] text-center">
                  {p.isApproved !== false && p.isApproved !== 'false' ? <span className="text-green-600 font-bold">Approved</span> : <span className="text-orange-500 font-bold">Pending</span>}
                  {p.aiFlagged && <div className="text-xs text-red-600 font-bold mt-1">Flagged by AI</div>}
                </td>

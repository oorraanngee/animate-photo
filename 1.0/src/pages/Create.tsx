...
export default function Create() {
  ...
  const handleCreate = async () => {
    if (!user) return alert(t('Please login'));
    if (isBanned) return alert(t('Banned'));
    if (frames.length === 0) return alert(t('Select Images'));
    if (frames.length === 1 && frames[0].file.type !== 'image/gif') return alert(t('Select Images'));
    
    const totalSize = frames.reduce((acc, frame) => acc + frame.file.size, 0);
    if (totalSize > 800000) return alert('Total file size must be less than 800KB');

    if (!title) return alert(t('Title') + ' is required');
    if (!db) return alert(t('Setup Required'));

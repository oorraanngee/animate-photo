    ...
    try {
      ...
      if (db) {
        ...
        if (userDoc.exists()) {
          ...
          const isAdmin = data.role === 'admin' || cred.user.email === 'dimabrovcuk4@gmail.com';
          setUser(cred.user, isAdmin, data.isBanned || false, true);
          navigate('/');
        } else {
      ...
      if (mode === 'register') {
        ...
        if (!userCred.user.emailVerified) {
          ...
          if (db) {
            ...
            if (userDoc.exists()) {
              const data = userDoc.data();
              const isAdmin = data.role === 'admin' || userCred.user.email === 'dimabrovcuk4@gmail.com';
              setUser(userCred.user, isAdmin, data.isBanned || false, true);
              navigate('/');
            } else {
